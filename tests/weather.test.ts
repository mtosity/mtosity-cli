...
describe("Weather Command", () => {
  ...
  test("should use Geocoding API when city and date provided", async () => {
    ...
    await showWeather(["Paris", "--date", "2026-12-25"]);
    ...
    expect(calls).toContain("Snow"); // date 2026-12-25 -> Snow (71)
  });

  test("should handle API errors gracefully", async () => {
    global.fetch = mock(async () => new Response("Body", { status: 500, statusText: "Internal Server Error" })) as any;
    
    await showWeather(["London"]);
    
    expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining("Error: Failed: Internal Server Error"));
  });
});
